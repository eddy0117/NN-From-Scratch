...
class Conv2d(BaseModule):
    ...
    def im2col(self, input_feat: np.ndarray, N, kh, kw, out_h, out_w, stride):
        ...
        input_feat = input_feat.transpose(0, 2, 3, 1)  # (N, H, W, C)
    ...
    def convolution(self, input_feat: np.ndarray, filter: np.ndarray, bias=None, stride=1, mode="standard"):
        ...
        out_feat = out_feat.reshape(out_c, N, out_h, out_w).transpose(1, 0, 2, 3)
    ...
    def t_convolution(self, input_feat: np.ndarray, filter: np.ndarray, stride=1, mode=None):
        ...
        if mode == "dLdW":
            ...
            #         -> (ori_c, kh, kw, convd_c)
            #         -> (convd_c, ori_c, kh, kw)
            out_feat = out_feat.reshape(-1, ori_h, ori_w, convd_c).transpose(3, 0, 1, 2)
        elif mode == "dLdZ":
